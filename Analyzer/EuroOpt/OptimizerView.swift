...
struct OptimizerView: View {
    ...
    private var robustnessSection: some View {
        GroupBox("🧪 Alpha 7.5 Robustheitsanalyse") {
            VStack(alignment: .leading, spacing: 10) {
                Text("Wiederholt die Validation/Holdout-Prüfung über fünf zeitlich getrennte Splits. Das Profil wird je Split ausschließlich aus der Validation gewählt und danach unverändert im Holdout geprüft.").foregroundStyle(.secondary)
                Text("Der Test ist separat: Der bestehende Produktions-WeightSweepEngine und das gespeicherte Profil werden nicht verändert. A100 wird wie jedes andere Profil behandelt.").font(.footnote).foregroundStyle(.secondary)

                Button { viewModel.runRobustnessAnalysis() } label: {
                    if viewModel.isRobustnessRunning { ProgressView().frame(maxWidth: .infinity) }
    ...
    private var f2AlphaFilterSection: some View {
        GroupBox("🛡️ F2/50 → Alpha Kontrollfilter") {
            VStack(alignment: .leading, spacing: 10) {
                Text("F2/50 bleibt immer die Basis. Alpha darf den F2-Tipp nur ersetzen, wenn der Alpha-Vorteil auf der Validation mindestens die geprüfte Schwelle erreicht.").foregroundStyle(.secondary)
                Text("Geprüfte Schwellen: 0,00 / 0,02 / 0,04 / 0,06 / 0,08 / 0,10 Δ. Der Holdout wird erst nach der jeweiligen Entscheidung ausgewertet.").font(.footnote).foregroundStyle(.secondary)

                Button {
                    startF2AlphaFilterAnalysis()
                } label: {
                    if f2AlphaFilterRunning { ProgressView().frame(maxWidth: .infinity) }
    ...
    private func startF2AlphaFilterAnalysis() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            F2AlphaFilterAnalyzer().run(draws: draws, recommendationCount: AppSettings.recommendationCount, splitCount: 10)
    ...
    private var recommendationsSection: some View {
        GroupBox("Empfehlungen") {
            if viewModel.reports.isEmpty {
                Text("Noch keine Empfehlungen vorhanden.").foregroundStyle(.secondary).frame(maxWidth: .infinity, alignment: .leading)
            } else {
                VStack(alignment: .leading, spacing: 14) {
