...
/// The analyzer studies pair co-occurrence among the Top-15 F2 numbers
/// and checks whether the relationship is stable across windows and time.
final class F2CorrelationAnalyzer {
    ...
    func run(draws: [EuroJackpotDraw]) {
        ...
        let block = max(1, total / splitCount)
        ...
        for split in 0..<splitCount {
            ...
            var byWindow: [Int: [PairKey: PairMetric]] = [:]

            for window in windows {
                let source = Array(training.suffix(window))
                byWindow[window] = pairMetrics(source: source, pool: top15)
            }

            snapshots.append(Snapshot(top15: top15, metricsByWindow: byWindow))
        ...
        var numberAppearances: [Int: Int] = [:]
        var numberRanks: [Int: [Int]] = [:]
        for snapshot in snapshots {
            for (rank, number) in snapshot.top15.enumerated() {
                numberAppearances[number, default: 0] += 1
                numberRanks[number, default: []].append(rank + 1)
            }
        }
        for number in numberAppearances.keys.sorted(by: { numberAppearances[$0]! > numberAppearances[$1]! }) {
            let ranks = numberRanks[number]!
            let meanRank = Double(ranks.reduce(0, +)) / Double(ranks.count)
            print(String(format: "%2d | in %2d/%d Snapshots | Ø Rang %.1f", number, numberAppearances[number]!, snapshots.count, meanRank))
        }

        let allPairs = allPairs(from: snapshots)
        ...
        for pair in allPairs {
            ...
            for snapshot in snapshots {
                for window in windows {
                    guard let metric = snapshot.metricsByWindow[window]?[pair] else { continue }
                    lifts.append(metric.lift)
                    phis.append(metric.phi)
                    totalCo += metric.coOccurrences
                    valid += 1
                    if metric.lift > 1.0 { positive += 1 }
                }
            }
        ...
        let strongest = summaries.sorted {
            if abs($0.1 - 1.0) != abs($1.1 - 1.0) {
                return abs($0.1 - 1.0) > abs($1.1 - 1.0)
            }
            return $0.2 > $1.2
        }
        ...
        for item in strongest.prefix(20) {
            print(String(format: "%2d-%2d | %6.3f | %+.3f | %2d     | %d", item.0.a, item.0.b, item.1, item.2, item.3, item.4))
        }

        let positivePairs = summaries.filter { $0.1 > 1.05 && $0.2 > 0.05 }
        let negativePairs = summaries.filter { $0.1 < 0.95 && $0.2 < -0.05 }
        print("\n## STABILITÄT")
        ...
        for (index, snapshot) in snapshots.enumerated() {
            let candidates = snapshot.metricsByWindow[f2Window] ?? [:]
            let stable = candidates.values
                .filter { $0.lift > 1.05 && $0.phi > 0.05 }
                .sorted { $0.lift > $1.lift }
                .prefix(5)
                .map { "\($0.pair.a)-\($0.pair.b) (L \(String(format: "%.2f", $0.lift)))" }
                .joined(separator: ", ")
            print("Split \(index + 1): \(stable.isEmpty ? "keine stabil positive Paarbeziehung im F2/50-Fenster" : stable)")
        }
    ...
    private func pairMetrics(source: [EuroJackpotDraw], pool: [Int]) -> [PairKey: PairMetric] {
        ...
        for draw in source {
            let present = Set(draw.numbers).intersection(pool)
        ...
        for i in 0..<pool.count {
            for j in (i + 1)..<pool.count {
                ...
                result[pair] = PairMetric(pair: pair, lift: lift, phi: phi, coOccurrences: Int(cab), support: support)
            }
        }
        return result
    }

    private func allPairs(from snapshots: [Snapshot]) -> Set<PairKey> {
        var pairs = Set<PairKey>()
        for snapshot in snapshots {
            for window in windows {
                pairs.formUnion(snapshot.metricsByWindow[window]?.keys ?? [])
            }
        }
        return pairs
    }
