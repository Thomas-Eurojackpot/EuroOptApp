...
final class F2CorrelationAnalyzer {
    ...
    func run(draws: [EuroJackpotDraw]) {
        ...
        for number in orderedNumbers {
            ...
            let meanRank = values.isEmpty ? 0.0 : Double(values.reduce(0, +)) / Double(values.count)
            print(String(format: "%2d | in %2d/%d Snapshots | Ø Rang %.1f", number, appearances[number, default: 0], snapshots.count, meanRank))
        }
        ...
        for snapshot in snapshots {
            for window in windows {
                if let keys = snapshot.metricsByWindow[window]?.keys {
                    for key in keys { pairSet.insert(key) }
                }
            }
        }
        for pair in pairSet {
        ...
        for index in snapshots.indices {
            ...
            let stable = metrics.values
                ...
                .prefix(5)
                .map {
                    let liftText = String(format: "%.2f", $0.lift)
                    return "\($0.pair.a)-\($0.pair.b) (L \(liftText))"
                }
                .joined(separator: ", ")
            let text = stable.isEmpty ? "keine stabil positive Paarbeziehung im F2/50-Fenster" : stable
        ...
        if pool.count >= 2 {
            for i in 0..<(pool.count - 1) {
                for j in (i + 1)..<pool.count {
                    ...
                    result[pair] = PairMetric(pair: pair, lift: lift, phi: phi, coOccurrences: Int(cab))
                }
            }
        }
        return result
