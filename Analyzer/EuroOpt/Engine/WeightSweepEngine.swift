...
final class WeightSweepEngine {
    ...
    private struct HitClassAggregate {
        ...
        func count(mainHits: Int, euroHits: Int) -> Int { counts[mainHits * 3 + euroHits] }
    ...
    private struct ExpectedHitClassAggregate {
        ...
        mutating func add(date: Date, ticketCount: Int) {
            ...
            for mainHits in 0...5 {
                let mainProbability = WeightSweepEngine.hypergeometricProbability(successPopulation: 5, failurePopulation: 45, draws: 5, successes: mainHits)
                for euroHits in 0...2 {
                    let euroProbability = WeightSweepEngine.hypergeometricProbability(successPopulation: 2, failurePopulation: euroCount - 2, draws: 2, successes: euroHits)
                    counts[mainHits * 3 + euroHits] += mainProbability * euroProbability * Double(ticketCount)
                }
            }
        }
        func expectedCount(mainHits: Int, euroHits: Int) -> Double { counts[mainHits * 3 + euroHits] }
        var total: Double { counts.reduce(0, +) }
    }
    ...
    func run(draws: [EuroJackpotDraw], recommendationCount: Int) {
        guard draws.count > 140 else { print("❌ Weight-Sweep: zu wenige Ziehungen"); return }
        let start = Date()
        ...
        for index in 100..<holdoutStart {
            ...
            if (index - 99).isMultiple(of: 50) { print("... Validation \(index - 99) / \(validationTests)") }
        }

        let ranked = profiles.indices.sorted { lhs, rhs in validationScore(validationTotals[lhs]) > validationScore(validationTotals[rhs]) }
        ...
        for (rank, profileIndex) in ranked.prefix(10).enumerated() {
            ...
            print(String(format: "%2d. P%02d  Haupt %.3f  Euro %.3f  Basis %.3f  Score %+.3f  %@", rank + 1, profiles[profileIndex].id, averageMain(total), averageEuro(total), averageExpectedEuro(total), validationScore(total), weightLabel(profiles[profileIndex])))
        }
        print("")
        print("🏆 GEWÄHLTES VALIDATION-PROFIL P\(String(format: "%02d", winner.id))")
        print(String(format: "Haupt %.3f | Euro %.3f | Euro-Basis %.3f | kombinierter Überschuss %+.3f", averageMain(winnerValidation), averageEuro(winnerValidation), averageExpectedEuro(winnerValidation), validationScore(winnerValidation)))
        ...
        for index in holdoutStart..<draws.count {
            ...
            if current.isMultiple(of: 50) { print("... Holdout \(current) / \(totalTests - validationTests)") }
        }
    ...
    func runGUConfirmation(draws: [EuroJackpotDraw], recommendationCount: Int, windowSize: Int = 100) {
        guard draws.count > windowSize + 100 else { print("❌ G/U-Bestätigung: zu wenige Ziehungen"); return }
        let start = Date()
        ...
        for index in firstIndex..<draws.count {
            ...
            if current.isMultiple(of: 25) { print("... Bestätigung \(current) / \(draws.count - firstIndex)") }
        }
    ...
    private func bestTickets(candidates: [Ticket], scoreEngine: ScoreEngine, limit: Int) -> [Ticket] {
        ...
        for candidate in scored {
            ...
            for existing in result {
                if commonNumbers(existing, candidate.0) >= 3 { different = false; break }
            }
            if different {
    ...
    private func averageMain(_ aggregate: Aggregate) -> Double { aggregate.tickets > 0 ? Double(aggregate.hits) / Double(aggregate.tickets) : 0 }
    private func averageEuro(_ aggregate: Aggregate) -> Double { aggregate.tickets > 0 ? Double(aggregate.euroHits) / Double(aggregate.tickets) : 0 }
    private func averageExpectedEuro(_ aggregate: Aggregate) -> Double { aggregate.tickets > 0 ? aggregate.expectedEuroHits / Double(aggregate.tickets) : 0 }
    private func validationScore(_ aggregate: Aggregate) -> Double {
        (averageMain(aggregate) - 0.50) + (averageEuro(aggregate) - averageExpectedEuro(aggregate))
    }
    ...
    private static func hypergeometricProbability(successPopulation: Int, failurePopulation: Int, draws: Int, successes: Int) -> Double {
        guard successes >= 0, successes <= draws, successes <= successPopulation, draws - successes <= failurePopulation else { return 0 }
        return combination(successPopulation, successes) * combination(failurePopulation, draws - successes) / combination(successPopulation + failurePopulation, draws)
    }
    private static func combination(_ n: Int, _ k: Int) -> Double {
        ...
        let m = min(k, n - k)
        var result = 1.0
        for i in 1...m { result *= Double(n - m + i) / Double(i) }
        return result
    }

    private func printHitClassEvaluation(_ aggregate: HitClassAggregate, expected: ExpectedHitClassAggregate, totalTickets: Int) {
        print("-----------------------------------")
        ...
        for mainHits in stride(from: 5, through: 0, by: -1) {
            for euroHits in stride(from: 2, through: 0, by: -1) {
                ...
                print(String(format: "%d + %d     %6d    %7.2f %%   %8.2f    %8.2f %%    %+7.2f", mainHits, euroHits, observed, observedShare, expectedCount, expectedShare, deltaShare))
            }
        }
        let winningTickets = (0...5).reduce(0) { partial, mainHits in partial + (0...2).reduce(0) { inner, euroHits in (mainHits > 0 || euroHits > 0) ? inner + aggregate.count(mainHits: mainHits, euroHits: euroHits) : inner } }
    ...
    private func makeProfiles() -> [Profile] {
        ...
        let singles: [[Double]] = [[100,0,0,0,0,0],[0,100,0,0,0,0],[0,0,100,0,0,0],[0,0,0,100,0,0],[0,0,0,0,100,0],[0,0,0,0,0,100]]
        for weights in singles { profiles.append(Profile(id: profiles.count + 1, goal: makeGoal(weights), weights: weights)) }
        for lhs in 0..<6 { for rhs in (lhs + 1)..<6 { var weights = Array(repeating: 0.0, count: 6); weights[lhs] = 50; weights[rhs] = 50; profiles.append(Profile(id: profiles.count + 1, goal: makeGoal(weights), weights: weights)) } }
        for a in 0..<4 { for b in (a + 1)..<5 { for c in (b + 1)..<6 { var weights = Array(repeating: 0.0, count: 6); weights[a] = 34; weights[b] = 33; weights[c] = 33; profiles.append(Profile(id: profiles.count + 1, goal: makeGoal(weights), weights: weights)) } } }
    ...
    private func makeGoal(_ weights: [Double]) -> OptimizationGoal {
        OptimizationGoal(frequencyWeight: weights[0], pairWeight: weights[1], evenOddWeight: weights[2], highLowWeight: weights[3], sumWeight: weights[4], gapWeight: weights[5])
    }
    private func weightLabel(_ profile: Profile) -> String {
        String(format: "F %.0f | P %.0f | G/U %.0f | H/N %.0f | S %.0f | A %.0f", profile.weights[0], profile.weights[1], profile.weights[2], profile.weights[3], profile.weights[4], profile.weights[5])
    }

    // MARK: - Parity bridge
    // Internal test-only bridge. The normal Alpha 7.5 run does not call this.
    func paritySelections(candidates: [Ticket], trainingDraws: [EuroJackpotDraw], recommendationCount: Int) -> [WeightSweepParitySelection] {
        guard !candidates.isEmpty else { return [] }
        let profiles = makeProfiles()
        let cache = ScoreCache(draws: trainingDraws)
        let scoreEngines = profiles.map { ScoreEngine(cache: cache, goal: $0.goal) }
        return profiles.indices.map { index in
            let best = bestTickets(candidates: candidates, scoreEngine: scoreEngines[index], limit: recommendationCount)
            return WeightSweepParitySelection(profileID: profiles[index].id, weights: profiles[index].weights, tickets: best)
        }
    }
}
