...
final class F2FeatureAnalyzer {
    ...
    private enum Rule {
        ...
        var label: String {
            switch self {
            case .sum(let value): return "Sum\(value)"
            case .even(let value): return "Even\(value)"
            case .high(let value): return "High\(value)"
            case .consecutive(let value): return "Adj\(value)"
            case .spread(let value): return "Spread\(value)"
            }
        }
    }
    ...
    func run(draws: [EuroJackpotDraw]) {
        ...
        for split in 0..<splitCount {
            ...
            let holdoutEnd = split == splitCount - 1
                ? draws.count
                : validationStart + block
    ...
    private func makeVariants() -> [Variant] {
        ...
        let singles: [(String, Rule)] = [
            ...
            ("High2", .high(2)),
            ("Adj0", .consecutive(0)),
            ("Spread20–29", .spread(1)),
            ("Spread30–39", .spread(2)),
            ("Spread40+", .spread(3))
        ]

        for item in singles {
            variants.append(Variant(name: "F2 + \(item.0)", rules: [item.1]))
        }
        ...
        for i in 0..<singles.count {
            for j in (i + 1)..<singles.count {
                variants.append(
                    Variant(
                        name: "F2 + \(singles[i].0) + \(singles[j].0)",
                        rules: [singles[i].1, singles[j].1]
                    )
                )
            }
        }
    ...
    private func makeVariantTickets(from draws: [EuroJackpotDraw], variants: [Variant]) -> [Ticket] {
        ...
        let baseline = Ticket(
            numbers: Array(rankedMain.prefix(5)).sorted(),
            euroNumbers: Array(rankedEuro.prefix(2)).sorted()
        )
        ...
        for variant in variants {
            if variant.rules.isEmpty {
                ...
                result.append(bestTicket(rankedMain: rankedMain, rankedEuro: rankedEuro, frequencies: main, rules: variant.rules, fallback: baseline))
            }
        }
    ...
    private func bestTicket(rankedMain: [Int],
                            rankedEuro: [Int],
                            frequencies: [Int: Int],
                            rules: [Rule],
                    ...
                    for c in (b + 1)..<(pool.count - 2) {
                        for d in (c + 1)..<(pool.count - 1) {
                            for e in (d + 1)..<pool.count {
                                ...
                                if best == nil || score > best!.1 || (score == best!.1 && numbers.lexicographicallyPrecedes(best!.0)) {
                                    best = (numbers, score)
                                }
                            }
                        }
    ...
    private func matches(numbers: [Int], rules: [Rule]) -> Bool {
        ...
        for rule in rules {
            switch rule {
            case .sum(let bucket):
                if !matchesSum(sum, bucket: bucket) { return false }
            case .even(let value):
                if even != value { return false }
            case .high(let value):
                if high != value { return false }
            case .consecutive(let value):
                if adjacent != value { return false }
            case .spread(let bucket):
                if !matchesSpread(spread, bucket: bucket) { return false }
            }
        }
    ...
    private func matchesSum(_ sum: Int, bucket: Int) -> Bool {
        switch bucket {
        case 0: return sum < 100
        case 1: return sum >= 100 && sum < 125
        case 2: return sum >= 125 && sum < 150
        default: return sum >= 150
        }
    }

    private func matchesSpread(_ spread: Int, bucket: Int) -> Bool {
        switch bucket {
        case 0: return spread < 20
        case 1: return spread >= 20 && spread < 30
        case 2: return spread >= 30 && spread < 40
        default: return spread >= 40
        }
    }
    ...
    private func makeF2Ticket(from draws: [EuroJackpotDraw]) -> Ticket {
        ...
        return Ticket(
            numbers: Array((1...50).sorted {
                main[$0, default: 0] == main[$1, default: 0]
                    ? $0 < $1
                    : main[$0, default: 0] > main[$1, default: 0]
            }.prefix(5)).sorted(),
            euroNumbers: Array((1...12).sorted {
                euro[$0, default: 0] == euro[$1, default: 0]
                    ? $0 < $1
                    : euro[$0, default: 0] > euro[$1, default: 0]
            }.prefix(2)).sorted()
    ...
    private func printSplitResults(_ results: [SplitResult]) {
        ...
        for (index, result) in results.enumerated() {
            ...
            print(String(format: "%2d | %-34s | %+.3f | %+.3f | %+.3f",
                         index + 1,
                         result.variant,
                         valAdvantage,
